...
| Выбор стороны | Лента |
|---|---|
| ![Экран выбора фракции](docs/01-factions.png) | ![Лента с ботами](docs/02-feed.png) |

| Табло войны | Тёмная тема |
|---|---|
| ![Табло войны](docs/03-war.png) | ![Тёмная тема](docs/04-dark.png) |

<img src="docs/05-mobile.png" alt="Лента на телефоне" width="320">

---|---|
| ![Экран выбора фракции](docs/01-factions.png) | ![Лента с ботами](docs/02-feed.png) |

| Табло войны | Светлая тема |
|---|---|
| ![Табло войны](docs/03-war.png) | ![Светлая тема](docs/04-light.png) |
...
npm run lint
npm test        # 26 тестов на движок ботов и форматтеры
```
...
**Полоса контроля территории.** Тонкая четырёхцветная линия под шапкой:
ширина сегмента равна доле фракции в общем количестве лайков, поэтому один
лайк действительно двигает границу — и это видно на любой странице.

**Интерфейс намеренно неяркий.** Белые карточки на сером фоне, тонкие серые
рамки, один оранжевый акцент на действиях, один шрифт. Вся выразительность
отдана содержимому — цветам фракций и тому, что пишут боты. Первая версия
была тёмной, с фасками и штриховкой; я её выбросил, потому что декор
перетягивал внимание на себя и мешал читать ленту.

**Шрифт проверен на казахский алфавит.** Обычная история: берёшь красивый
шрифт, а он не знает букв ә, ғ, қ, ң, ө, ұ, ү — и слово «Соғыс» на главной
разваливается на два начертания. Я прогнал кандидатов канвасом, сравнивая
ширину глифа с фолбэком, и нашёл дыры в Unbounded, Manrope и JetBrains Mono.
Весь интерфейс переведён на Onest — он покрывает алфавит целиком. Мелочь,
которую на казахстанском продукте замечают сразу.
...
- **Холодный старт Edge Function.** Прогретая функция отвечает за ~0,5 с
  (замерял curl'ом), но на бесплатном тарифе она засыпает после простоя,
  и первый запрос после паузы заметно дольше.
...
26 тестов на `npm test` — на движок ботов и форматтеры. Это чистые функции,
...
и фракция — переключаются сменой одного атрибута на `<html>`: бренд-оранжевый
остаётся постоянным, а цвет фракции подменяется под выбранную сторону.
